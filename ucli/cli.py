...
@admin.command(name = "platformReportStats", help="Get stats for an overall platform report.")
...
def problem(problem_alias, token, username):
    ctx = get_client()
    api_dict = ctx.authorization.problem(problem_alias=problem_alias, token=token, username=username)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@badge.command(name = "badgeDetails", help="Returns the number of owners and the first assignation timestamp for a certain badge")
...
@badge.command(name = "myBadgeAssignationTime", help="Returns a the assignation timestamp of a badge for current user.")
@click.option("--badge-alias", default=None, type=str)
...
@clarification.command(name = "create", help="Creates a Clarification for a contest or an assignment of a course")
...
def create(message, problem_alias, assignment_alias, contest_alias, course_alias, username):
    ctx = get_client()
    api_dict = ctx.clarification.create(message=message, problem_alias=problem_alias, assignment_alias=assignment_alias, contest_alias=contest_alias, course_alias=course_alias, username=username)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def update(clarification_id, answer, message, public):
    ctx = get_client()
    api_dict = ctx.clarification.update(clarification_id=clarification_id, answer=answer, message=message, public=public)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def details(contest_alias, raw):
    ...
    start_time = cli_terminal.greenyellow(
        str(datetime.datetime.fromtimestamp(api_dict["start_time"]))
    )
    finish_time = cli_terminal.greenyellow(
        str(datetime.datetime.fromtimestamp(api_dict["finish_time"]))
    )
...
@contest.command(name = "activityReport", help="Returns a report with all user activity for a contest.")
@click.argument("contest_alias")
...
def activity_report(contest_alias, length, page, token):
    ctx = get_client()
    api_dict = ctx.contest.activityReport(contest_alias=contest_alias, length=length, page=page, token=token)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def add_admin(contest_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.contest.addAdmin(contest_alias=contest_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def add_problem(contest_alias, order_in_contest, points, problem_alias, commit):
    ctx = get_client()
    api_dict = ctx.contest.addProblem(contest_alias=contest_alias, order_in_contest=order_in_contest, points=points, problem_alias=problem_alias, commit=commit)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@contest.command(name = "addUser", help="Adds a user to a contest. By default, any user can view details of public contests. Only users added through this API can view private contests")
@click.argument("contest_alias")
@click.argument("username_or_email")
def add_user(contest_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.contest.addUser(contest_alias=contest_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@contest.command(name = "adminDetails", help="Returns details of a Contest, for administrators. This differs from apiDetails in the sense that it does not attempt to calculate the remaining time from the contest, or register the opened time.")
@click.argument("contest_alias")
@click.option("--token", default=None, type=str)
...
@contest.command(name = "adminList", help="Returns a list of contests where current user has admin rights (or is the director).")
@click.option("--page", default=None, type=int)
...
def admin_list(page, page_size, show_archived):
    ctx = get_client()
    api_dict = ctx.contest.adminList(page=page, page_size=page_size, show_archived=show_archived)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def arbitrate_request(contest_alias, username, note, resolution):
    ctx = get_client()
    api_dict = ctx.contest.arbitrateRequest(contest_alias=contest_alias, username=username, note=note, resolution=resolution)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@contest.command(name = "archive", help="Archives or Unarchives a contest if user is the creator")
@click.argument("contest_alias")
...
def clarifications(contest_alias, offset, rowcount):
    ctx = get_client()
    api_dict = ctx.contest.clarifications(contest_alias=contest_alias, offset=offset, rowcount=rowcount)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def clone(contest_alias, description, start_time, title, alias, auth_token):
    ctx = get_client()
    api_dict = ctx.contest.clone(contest_alias=contest_alias, description=description, start_time=start_time, title=title, alias=alias, auth_token=auth_token)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@contest.command(name = "contestants", help="Return users who participate in a contest, as long as contest admin has chosen to ask for users information and contestants have previously agreed to share their information.")
@click.argument("contest_alias")
...
def create(admission_mode, alias, contest_for_teams, description, feedback, finish_time, languages, needs_basic_information, partial_score, penalty, penalty_calc_policy, penalty_type, points_decay_factor, problems, requests_user_information, scoreboard, show_scoreboard_after, start_time, submissions_gap, teams_group_alias, title, window_length):
    ctx = get_client()
    api_dict = ctx.contest.create(admission_mode=admission_mode, alias=alias, contest_for_teams=contest_for_teams, description=description, feedback=feedback, finish_time=finish_time, languages=languages, needs_basic_information=needs_basic_information, partial_score=partial_score, penalty=penalty, penalty_calc_policy=penalty_calc_policy, penalty_type=penalty_type, points_decay_factor=points_decay_factor, problems=problems, requests_user_information=requests_user_information, scoreboard=scoreboard, show_scoreboard_after=show_scoreboard_after, start_time=start_time, submissions_gap=submissions_gap, teams_group_alias=teams_group_alias, title=title, window_length=window_length)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def list_(page, page_size, query, tab_name, active, admission_mode, participating, recommended):
    ctx = get_client()
    api_dict = ctx.contest.list(page=page, page_size=page_size, query=query, tab_name=tab_name, active=active, admission_mode=admission_mode, participating=participating, recommended=recommended)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@contest.command(name = "listParticipating", help="Returns a list of contests where current user is participating in")
@click.option("--page", default=None, type=int)
@click.option("--page-size", default=None, type=int)
...
def list_participating(page, page_size, query, show_archived):
    ctx = get_client()
    api_dict = ctx.contest.listParticipating(page=page, page_size=page_size, query=query, show_archived=show_archived)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@contest.command(name = "myList", help="Returns a list of contests where current user is the director")
@click.option("--page", default=None, type=int)
@click.option("--page-size", default=None, type=int)
@click.option("--query", default=None, type=str)
@click.option("--show-archived", default=None, type=bool)
def my_list(page, page_size, query, show_archived):
    ctx = get_client()
    api_dict = ctx.contest.myList(page=page, page_size=page_size, query=query, show_archived=show_archived)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@contest.command(name = "open", help="Joins a contest - explicitly adds a identity to a contest.")
@click.argument("contest_alias")
...
@click.option("--token", default=None, type=str)
def open_(contest_alias, privacy_git_object_id, statement_type, share_user_information, token):
    ctx = get_client()
    api_dict = ctx.contest.open(contest_alias=contest_alias, privacy_git_object_id=privacy_git_object_id, statement_type=statement_type, share_user_information=share_user_information, token=token)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@contest.command(name = "problemClarifications", help="Get clarifications of problem in a contest")
@click.argument("contest_alias")
@click.argument("offset")
@click.argument("problem_alias")
@click.argument("rowcount")
def problem_clarifications(contest_alias, offset, problem_alias, rowcount):
    ctx = get_client()
    api_dict = ctx.contest.problemClarifications(contest_alias=contest_alias, offset=offset, problem_alias=problem_alias, rowcount=rowcount)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def remove_admin(contest_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.contest.removeAdmin(contest_alias=contest_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def remove_problem(contest_alias, problem_alias):
    ctx = get_client()
    api_dict = ctx.contest.removeProblem(contest_alias=contest_alias, problem_alias=problem_alias)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def remove_user(contest_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.contest.removeUser(contest_alias=contest_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@contest.command(name = "replaceTeamsGroup", help="Replace the teams group assigned to a contest")
@click.argument("contest_alias")
@click.argument("teams_group_alias")
def replace_teams_group(contest_alias, teams_group_alias):
    ctx = get_client()
    api_dict = ctx.contest.replaceTeamsGroup(contest_alias=contest_alias, teams_group_alias=teams_group_alias)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def report(contest_alias, auth_token, filter_by):
    ctx = get_client()
    api_dict = ctx.contest.report(contest_alias=contest_alias, auth_token=auth_token, filterBy=filter_by)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def runs(contest_alias, problem_alias, language, offset, rowcount, status, username, verdict):
    ctx = get_client()
    api_dict = ctx.contest.runs(contest_alias=contest_alias, problem_alias=problem_alias, language=language, offset=offset, rowcount=rowcount, status=status, username=username, verdict=verdict)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@contest.command(name = "runsDiff", help="Return a report of which runs would change due to a version change.")
@click.argument("contest_alias")
@click.argument("version")
@click.option("--problem-alias", default=None, type=str)
def runs_diff(contest_alias, version, problem_alias):
    ctx = get_client()
    api_dict = ctx.contest.runsDiff(contest_alias=contest_alias, version=version, problem_alias=problem_alias)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@contest.command(name = "scoreboardMerge", help="Gets the accomulative scoreboard for an array of contests")
...
def scoreboard_merge(contest_aliases, contest_params, usernames_filter):
    ctx = get_client()
    api_dict = ctx.contest.scoreboardMerge(contest_aliases=contest_aliases, contest_params=contest_params, usernames_filter=usernames_filter)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@contest.command(name = "setRecommended", help="Given a contest_alias, sets the recommended flag on/off. Only omegaUp admins can call this API.")
@click.argument("contest_alias")
...
def update(contest_alias, finish_time, submissions_gap, window_length, admission_mode, alias, contest_for_teams, default_show_all_contestants_in_scoreboard, description, feedback, languages, needs_basic_information, partial_score, penalty, penalty_calc_policy, penalty_type, points_decay_factor, problems, requests_user_information, scoreboard, show_scoreboard_after, start_time, teams_group_alias, title):
    ctx = get_client()
    api_dict = ctx.contest.update(contest_alias=contest_alias, finish_time=finish_time, submissions_gap=submissions_gap, window_length=window_length, admission_mode=admission_mode, alias=alias, contest_for_teams=contest_for_teams, default_show_all_contestants_in_scoreboard=default_show_all_contestants_in_scoreboard, description=description, feedback=feedback, languages=languages, needs_basic_information=needs_basic_information, partial_score=partial_score, penalty=penalty, penalty_calc_policy=penalty_calc_policy, penalty_type=penalty_type, points_decay_factor=points_decay_factor, problems=problems, requests_user_information=requests_user_information, scoreboard=scoreboard, show_scoreboard_after=show_scoreboard_after, start_time=start_time, teams_group_alias=teams_group_alias, title=title)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@contest.command(name = "updateEndTimeForIdentity", help="Update Contest end time for an identity when window_length option is turned on")
@click.argument("contest_alias")
@click.argument("end_time")
@click.argument("username")
def update_end_time_for_identity(contest_alias, end_time, username):
    ctx = get_client()
    api_dict = ctx.contest.updateEndTimeForIdentity(contest_alias=contest_alias, end_time=end_time, username=username)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@course.command(name = "activityReport", help="Returns a report with all user activity for a course.")
@click.argument("course_alias")
@click.option("--length", default=None, type=int)
@click.option("--page", default=None, type=int)
def activity_report(course_alias, length, page):
    ctx = get_client()
    api_dict = ctx.course.activityReport(course_alias=course_alias, length=length, page=page)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def add_admin(course_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.course.addAdmin(course_alias=course_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def add_problem(assignment_alias, course_alias, points, problem_alias, commit, is_extra_problem):
    ctx = get_client()
    api_dict = ctx.course.addProblem(assignment_alias=assignment_alias, course_alias=course_alias, points=points, problem_alias=problem_alias, commit=commit, is_extra_problem=is_extra_problem)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def add_student(accept_teacher_git_object_id, course_alias, privacy_git_object_id, share_user_information, statement_type, username_or_email, accept_teacher):
    ctx = get_client()
    api_dict = ctx.course.addStudent(accept_teacher_git_object_id=accept_teacher_git_object_id, course_alias=course_alias, privacy_git_object_id=privacy_git_object_id, share_user_information=share_user_information, statement_type=statement_type, usernameOrEmail=username_or_email, accept_teacher=accept_teacher)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@course.command(name = "arbitrateRequest", help="Stores the resolution given to a certain request made by a contestant interested to join the course.")
@click.argument("course_alias")
@click.argument("resolution")
@click.argument("username")
def arbitrate_request(course_alias, resolution, username):
    ctx = get_client()
    api_dict = ctx.course.arbitrateRequest(course_alias=course_alias, resolution=resolution, username=username)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def assignment_details(assignment, course, token):
    ctx = get_client()
    api_dict = ctx.course.assignmentDetails(assignment=assignment, course=course, token=token)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def assignment_scoreboard(assignment, course, token):
    ctx = get_client()
    api_dict = ctx.course.assignmentScoreboard(assignment=assignment, course=course, token=token)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def assignment_scoreboard_events(assignment, course, token):
    ctx = get_client()
    api_dict = ctx.course.assignmentScoreboardEvents(assignment=assignment, course=course, token=token)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@course.command(name = "clarifications", help="Gets the clarifications of all assignments in a course")
@click.argument("course_alias")
@click.argument("offset")
@click.argument("rowcount")
def clarifications(course_alias, offset, rowcount):
    ctx = get_client()
    api_dict = ctx.course.clarifications(course_alias=course_alias, offset=offset, rowcount=rowcount)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def clone(alias, course_alias, name, start_time, token):
    ctx = get_client()
    api_dict = ctx.course.clone(alias=alias, course_alias=course_alias, name=name, start_time=start_time, token=token)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def create(admission_mode, alias, description, finish_time, languages, level, name, needs_basic_information, objective, public, requests_user_information, school_id, show_scoreboard, start_time, unlimited_duration):
    ctx = get_client()
    api_dict = ctx.course.create(admission_mode=admission_mode, alias=alias, description=description, finish_time=finish_time, languages=languages, level=level, name=name, needs_basic_information=needs_basic_information, objective=objective, public=public, requests_user_information=requests_user_information, school_id=school_id, show_scoreboard=show_scoreboard, start_time=start_time, unlimited_duration=unlimited_duration)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@click.option("--start-time", default=None)
@click.option("--unlimited-duration", default=None, type=bool)
def create_assignment(course_alias, alias, assignment_type, description, finish_time, name, order, problems, publish_time_delay, start_time, unlimited_duration):
    ctx = get_client()
    api_dict = ctx.course.createAssignment(course_alias=course_alias, alias=alias, assignment_type=assignment_type, description=description, finish_time=finish_time, name=name, order=order, problems=problems, publish_time_delay=publish_time_delay, start_time=start_time, unlimited_duration=unlimited_duration)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def get_problem_users(course_alias, problem_alias):
    ctx = get_client()
    api_dict = ctx.course.getProblemUsers(course_alias=course_alias, problem_alias=problem_alias)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@course.command(name = "introDetails", help="Show course intro only on public courses when user is not yet registered")
@click.argument("course_alias")
...
@course.command(name = "listSolvedProblems", help="Get Problems solved by users of a course")
@click.argument("course_alias")
...
@course.command(name = "listUnsolvedProblems", help="Get Problems unsolved by users of a course")
@click.argument("course_alias")
...
@course.command(name = "problemClarifications", help="Get clarifications of problem in a contest")
@click.argument("assignment_alias")
@click.argument("course_alias")
@click.argument("offset")
@click.argument("problem_alias")
@click.argument("rowcount")
def problem_clarifications(assignment_alias, course_alias, offset, problem_alias, rowcount):
    ctx = get_client()
    api_dict = ctx.course.problemClarifications(assignment_alias=assignment_alias, course_alias=course_alias, offset=offset, problem_alias=problem_alias, rowcount=rowcount)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def register_for_course(course_alias, accept_teacher, share_user_information):
    ctx = get_client()
    api_dict = ctx.course.registerForCourse(course_alias=course_alias, accept_teacher=accept_teacher, share_user_information=share_user_information)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def remove_admin(course_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.course.removeAdmin(course_alias=course_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def remove_assignment(assignment_alias, course_alias):
    ctx = get_client()
    api_dict = ctx.course.removeAssignment(assignment_alias=assignment_alias, course_alias=course_alias)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def remove_problem(assignment_alias, course_alias, problem_alias):
    ctx = get_client()
    api_dict = ctx.course.removeProblem(assignment_alias=assignment_alias, course_alias=course_alias, problem_alias=problem_alias)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def remove_student(course_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.course.removeStudent(course_alias=course_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@course.command(name = "requests", help="Returns the list of requests made by participants who are interested to join the course")
@click.argument("course_alias")
...
@click.option("--username", default=None, type=str)
@click.option("--verdict", default=None, type=str)
def runs(assignment_alias, course_alias, language, offset, problem_alias, rowcount, status, username, verdict):
    ctx = get_client()
    api_dict = ctx.course.runs(assignment_alias=assignment_alias, course_alias=course_alias, language=language, offset=offset, problem_alias=problem_alias, rowcount=rowcount, status=status, username=username, verdict=verdict)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def student_progress(assignment_alias, course_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.course.studentProgress(assignment_alias=assignment_alias, course_alias=course_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@click.option("--start-time", default=None, type=datetime.datetime)
@click.option("--unlimited-duration", default=None, type=bool)
def update(alias, languages, school_id, admission_mode, description, finish_time, level, name, needs_basic_information, objective, requests_user_information, show_scoreboard, start_time, unlimited_duration):
    ctx = get_client()
    api_dict = ctx.course.update(alias=alias, languages=languages, school_id=school_id, admission_mode=admission_mode, description=description, finish_time=finish_time, level=level, name=name, needs_basic_information=needs_basic_information, objective=objective, requests_user_information=requests_user_information, show_scoreboard=show_scoreboard, start_time=start_time, unlimited_duration=unlimited_duration)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def update_assignment(assignment, course, finish_time, start_time, unlimited_duration):
    ctx = get_client()
    api_dict = ctx.course.updateAssignment(assignment=assignment, course=course, finish_time=finish_time, start_time=start_time, unlimited_duration=unlimited_duration)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def update_assignments_order(assignments, course_alias):
    ctx = get_client()
    api_dict = ctx.course.updateAssignmentsOrder(assignments=assignments, course_alias=course_alias)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def update_problems_order(assignment_alias, course_alias, problems):
    ctx = get_client()
    api_dict = ctx.course.updateProblemsOrder(assignment_alias=assignment_alias, course_alias=course_alias, problems=problems)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def add_user(group_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.group.addUser(group_alias=group_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def create_scoreboard(group_alias, name, alias, description):
    ctx = get_client()
    api_dict = ctx.group.createScoreboard(group_alias=group_alias, name=name, alias=alias, description=description)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@group.command(name = "list", help="Returns a list of groups that match a partial name. This returns an array instead of an object since it is used by typeahead.")
@click.option("--query", default=None, type=str)
def list_(query):
    ctx = get_client()
...
def remove_user(group_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.group.removeUser(group_alias=group_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def add_contest(contest_alias, group_alias, scoreboard_alias, weight, only_ac):
    ctx = get_client()
    api_dict = ctx.groupScoreboard.addContest(contest_alias=contest_alias, group_alias=group_alias, scoreboard_alias=scoreboard_alias, weight=weight, only_ac=only_ac)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@group_scoreboard.command(name = "details", help="Details of a scoreboard. Returns a list with all contests that belong to the given scoreboard_alias")
@click.argument("group_alias")
@click.argument("scoreboard_alias")
def details(group_alias, scoreboard_alias):
    ctx = get_client()
    api_dict = ctx.groupScoreboard.details(group_alias=group_alias, scoreboard_alias=scoreboard_alias)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@group_scoreboard.command(name = "removeContest", help="Add contest to a group scoreboard")
@click.argument("contest_alias")
@click.argument("group_alias")
@click.argument("scoreboard_alias")
def remove_contest(contest_alias, group_alias, scoreboard_alias):
    ctx = get_client()
    api_dict = ctx.groupScoreboard.removeContest(contest_alias=contest_alias, group_alias=group_alias, scoreboard_alias=scoreboard_alias)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def bulk_create(identities, group_alias, name, username):
    ctx = get_client()
    api_dict = ctx.identity.bulkCreate(identities=identities, group_alias=group_alias, name=name, username=username)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@identity.command(name = "bulkCreateForTeams", help="Entry point for Create bulk Identities for teams API")
@click.argument("team_group_alias")
@click.argument("team_identities")
def bulk_create_for_teams(team_group_alias, team_identities):
    ctx = get_client()
    api_dict = ctx.identity.bulkCreateForTeams(team_group_alias=team_group_alias, team_identities=team_identities)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@identity.command(name = "changePassword", help="Entry point for change passowrd of an identity")
@click.argument("group_alias")
...
def change_password(group_alias, password, username, identities, name):
    ctx = get_client()
    api_dict = ctx.identity.changePassword(group_alias=group_alias, password=password, username=username, identities=identities, name=name)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def create(gender, name, password, school_name, username, country_id, group_alias, identities, state_id):
    ctx = get_client()
    api_dict = ctx.identity.create(gender=gender, name=name, password=password, school_name=school_name, username=username, country_id=country_id, group_alias=group_alias, identities=identities, state_id=state_id)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@identity.command(name = "selectIdentity", help="Entry point for switching between associated identities for a user")
@click.argument("username_or_email")
@click.option("--auth-token", default=None, type=str)
def select_identity(username_or_email, auth_token):
    ctx = get_client()
    api_dict = ctx.identity.selectIdentity(usernameOrEmail=username_or_email, auth_token=auth_token)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@click.option("--identities", default=None)
@click.option("--state-id", default=None, type=str)
def update(gender, group_alias, name, original_username, school_name, username, country_id, identities, state_id):
    ctx = get_client()
    api_dict = ctx.identity.update(gender=gender, group_alias=group_alias, name=name, original_username=original_username, school_name=school_name, username=username, country_id=country_id, identities=identities, state_id=state_id)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@identity.command(name = "updateIdentityTeam", help="Entry point for Update an Identity team API")
@click.argument("gender")
@click.argument("group_alias")
@click.argument("name")
@click.argument("original_username")
...
@click.option("--identities", default=None)
@click.option("--state-id", default=None, type=str)
def update_identity_team(gender, group_alias, name, original_username, school_name, username, country_id, identities, state_id):
    ctx = get_client()
    api_dict = ctx.identity.updateIdentityTeam(gender=gender, group_alias=group_alias, name=name, original_username=original_username, school_name=school_name, username=username, country_id=country_id, identities=identities, state_id=state_id)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@notification.command(name = "myList", help="Returns a list of unread notifications for user")
def my_list():
    ctx = get_client()
    api_dict = ctx.notification.myList()
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@notification.command(name = "readNotifications", help="Updates notifications as read in database")
...
def runs(problem_alias, raw):
    ...
    for i_run in api_dict["runs"]:
        ...
        submit_date = str(datetime.datetime.fromtimestamp(i_run["time"]))
...
def add_admin(problem_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.problem.addAdmin(problem_alias=problem_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@problem.command(name = "adminList", help="Returns a list of problems where current user has admin rights (or is the owner).")
@click.argument("page")
@click.argument("page_size")
@click.option("--query", default=None, type=str)
...
def best_score(contest_alias, problem_alias, problemset_id, statement_type, username):
    ctx = get_client()
    api_dict = ctx.problem.bestScore(contest_alias=contest_alias, problem_alias=problem_alias, problemset_id=problemset_id, statement_type=statement_type, username=username)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@problem.command(name = "clarifications", help="Entry point for Problem clarifications API")
@click.argument("problem_alias")
@click.option("--offset", default=None)
@click.option("--rowcount", default=None)
def clarifications(problem_alias, offset, rowcount):
    ctx = get_client()
    api_dict = ctx.problem.clarifications(problem_alias=problem_alias, offset=offset, rowcount=rowcount)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def create(problem_alias, visibility, allow_user_add_tags, email_clarifications, extra_wall_time, group_score_policy, input_limit, languages, memory_limit, output_limit, overall_wall_time_limit, problem_level, selected_tags, show_diff, source, time_limit, title, update_published, validator, validator_time_limit):
    ctx = get_client()
    api_dict = ctx.problem.create(problem_alias=problem_alias, visibility=visibility, allow_user_add_tags=allow_user_add_tags, email_clarifications=email_clarifications, extra_wall_time=extra_wall_time, group_score_policy=group_score_policy, input_limit=input_limit, languages=languages, memory_limit=memory_limit, output_limit=output_limit, overall_wall_time_limit=overall_wall_time_limit, problem_level=problem_level, selected_tags=selected_tags, show_diff=show_diff, source=source, time_limit=time_limit, title=title, update_published=update_published, validator=validator, validator_time_limit=validator_time_limit)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@click.option("--statement-type", default=None, type=str)
def details(problem_alias, contest_alias, lang, prevent_problemset_open, problemset_id, show_solvers, statement_type):
    ctx = get_client()
    api_dict = ctx.problem.details(problem_alias=problem_alias, contest_alias=contest_alias, lang=lang, prevent_problemset_open=prevent_problemset_open, problemset_id=problemset_id, show_solvers=show_solvers, statement_type=statement_type)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def list_(only_quality_seal, difficulty, difficulty_range, language, level, max_difficulty, min_difficulty, min_visibility, offset, only_karel, order_by, page, programming_languages, query, require_all_tags, rowcount, some_tags, sort_order):
    ctx = get_client()
    api_dict = ctx.problem.list(only_quality_seal=only_quality_seal, difficulty=difficulty, difficulty_range=difficulty_range, language=language, level=level, max_difficulty=max_difficulty, min_difficulty=min_difficulty, min_visibility=min_visibility, offset=offset, only_karel=only_karel, order_by=order_by, page=page, programming_languages=programming_languages, query=query, require_all_tags=require_all_tags, rowcount=rowcount, some_tags=some_tags, sort_order=sort_order)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@problem.command(name = "myList", help="Gets a list of problems where current user is the owner")
@click.argument("page")
@click.option("--query", default=None, type=str)
@click.option("--rowcount", default=None, type=int)
...
def remove_admin(problem_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.problem.removeAdmin(problem_alias=problem_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@problem.command(name = "runsDiff", help="Return a report of which runs would change due to a version change.")
@click.argument("version")
@click.option("--problem-alias", default=None, type=str)
...
def select_version(commit, problem_alias, update_published):
    ctx = get_client()
    api_dict = ctx.problem.selectVersion(commit=commit, problem_alias=problem_alias, update_published=update_published)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@problem.command(name = "solution", help="Returns the solution for a problem if conditions are satisfied.")
@click.option("--contest-alias", default=None, type=str)
...
@click.option("--problemset-id", default=None)
@click.option("--statement-type", default=None, type=str)
def solution(contest_alias, forfeit_problem, lang, problem_alias, problemset_id, statement_type):
    ctx = get_client()
    api_dict = ctx.problem.solution(contest_alias=contest_alias, forfeit_problem=forfeit_problem, lang=lang, problem_alias=problem_alias, problemset_id=problemset_id, statement_type=statement_type)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def tags(problem_alias, include_voted):
    ctx = get_client()
    api_dict = ctx.problem.tags(problem_alias=problem_alias, include_voted=include_voted)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def update(message, problem_alias, allow_user_add_tags, email_clarifications, extra_wall_time, group_score_policy, input_limit, languages, memory_limit, output_limit, overall_wall_time_limit, problem_level, redirect, selected_tags, show_diff, source, time_limit, title, update_published, validator, validator_time_limit, visibility):
    ctx = get_client()
    api_dict = ctx.problem.update(message=message, problem_alias=problem_alias, allow_user_add_tags=allow_user_add_tags, email_clarifications=email_clarifications, extra_wall_time=extra_wall_time, group_score_policy=group_score_policy, input_limit=input_limit, languages=languages, memory_limit=memory_limit, output_limit=output_limit, overall_wall_time_limit=overall_wall_time_limit, problem_level=problem_level, redirect=redirect, selected_tags=selected_tags, show_diff=show_diff, source=source, time_limit=time_limit, title=title, update_published=update_published, validator=validator, validator_time_limit=validator_time_limit, visibility=visibility)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@problem.command(name = "updateProblemLevel", help="Updates the problem level of a problem")
@click.argument("problem_alias")
@click.option("--level-tag", default=None, type=str)
def update_problem_level(problem_alias, level_tag):
    ctx = get_client()
    api_dict = ctx.problem.updateProblemLevel(problem_alias=problem_alias, level_tag=level_tag)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@click.option("--validator", default=None, type=str)
@click.option("--validator-time-limit", default=None)
def update_solution(message, problem_alias, solution, visibility, allow_user_add_tags, email_clarifications, extra_wall_time, group_score_policy, input_limit, lang, languages, memory_limit, output_limit, overall_wall_time_limit, problem_level, selected_tags, show_diff, source, time_limit, title, update_published, validator, validator_time_limit):
    ctx = get_client()
    api_dict = ctx.problem.updateSolution(message=message, problem_alias=problem_alias, solution=solution, visibility=visibility, allow_user_add_tags=allow_user_add_tags, email_clarifications=email_clarifications, extra_wall_time=extra_wall_time, group_score_policy=group_score_policy, input_limit=input_limit, lang=lang, languages=languages, memory_limit=memory_limit, output_limit=output_limit, overall_wall_time_limit=overall_wall_time_limit, problem_level=problem_level, selected_tags=selected_tags, show_diff=show_diff, source=source, time_limit=time_limit, title=title, update_published=update_published, validator=validator, validator_time_limit=validator_time_limit)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@click.option("--validator", default=None, type=str)
@click.option("--validator-time-limit", default=None)
def update_statement(message, problem_alias, statement, visibility, allow_user_add_tags, email_clarifications, extra_wall_time, group_score_policy, input_limit, lang, languages, memory_limit, output_limit, overall_wall_time_limit, problem_level, selected_tags, show_diff, source, time_limit, title, update_published, validator, validator_time_limit):
    ctx = get_client()
    api_dict = ctx.problem.updateStatement(message=message, problem_alias=problem_alias, statement=statement, visibility=visibility, allow_user_add_tags=allow_user_add_tags, email_clarifications=email_clarifications, extra_wall_time=extra_wall_time, group_score_policy=group_score_policy, input_limit=input_limit, lang=lang, languages=languages, memory_limit=memory_limit, output_limit=output_limit, overall_wall_time_limit=overall_wall_time_limit, problem_level=problem_level, selected_tags=selected_tags, show_diff=show_diff, source=source, time_limit=time_limit, title=title, update_published=update_published, validator=validator, validator_time_limit=validator_time_limit)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def versions(problem_alias, problemset_id):
    ctx = get_client()
    api_dict = ctx.problem.versions(problem_alias=problem_alias, problemset_id=problemset_id)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@problem_forfeited.command(name = "getCounts", help="Returns the number of solutions allowed and the number of solutions already seen")
def get_counts():
    ctx = get_client()
...
def details(assignment, contest_alias, course, problemset_id, auth_token, token, tokens):
    ctx = get_client()
    api_dict = ctx.problemset.details(assignment=assignment, contest_alias=contest_alias, course=course, problemset_id=problemset_id, auth_token=auth_token, token=token, tokens=tokens)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@click.option("--tokens", default=None)
def scoreboard(assignment, contest_alias, course, problemset_id, auth_token, token, tokens):
    ctx = get_client()
    api_dict = ctx.problemset.scoreboard(assignment=assignment, contest_alias=contest_alias, course=course, problemset_id=problemset_id, auth_token=auth_token, token=token, tokens=tokens)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@click.option("--token", default=None)
@click.option("--tokens", default=None)
def scoreboard_events(assignment, contest_alias, course, problemset_id, auth_token, token, tokens):
    ctx = get_client()
    api_dict = ctx.problemset.scoreboardEvents(assignment=assignment, contest_alias=contest_alias, course=course, problemset_id=problemset_id, auth_token=auth_token, token=token, tokens=tokens)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def list_(offset, rowcount, column, query, status):
    ctx = get_client()
    api_dict = ctx.qualityNomination.list(offset=offset, rowcount=rowcount, column=column, query=query, status=status)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@quality_nomination.command(name = "myAssignedList", help="Displays the nominations that this user has been assigned.")
@click.argument("page")
@click.argument("page_size")
...
@quality_nomination.command(name = "resolve", help="Marks a problem of a nomination (only the demotion type supported for now) as (resolved, banned, warning).")
@click.argument("problem_alias")
@click.argument("qualitynomination_id")
...
def resolve(problem_alias, qualitynomination_id, rationale, status, all_):
    ctx = get_client()
    api_dict = ctx.qualityNomination.resolve(problem_alias=problem_alias, qualitynomination_id=qualitynomination_id, rationale=rationale, status=status, all=all_)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@reset.command(name = "create", help="Creates a reset operation, the first of two steps needed to reset a password. The first step consist of sending an email to the user with instructions to reset he's password, if and only if the email is valid.")
...
@reset.command(name = "generateToken", help="Creates a reset operation, support team members can generate a valid token and then they can send it to end user")
@click.argument("email")
...
@reset.command(name = "update", help="Updates the password of a given user, this is the second and last step in order to reset the password. This operation is done if and only if the correct parameters are suplied.")
@click.argument("email")
@click.argument("password")
...
def update(email, password, password_confirmation, reset_token):
    ctx = get_client()
    api_dict = ctx.reset.update(email=email, password=password, password_confirmation=password_confirmation, reset_token=reset_token)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@run.command(name = "details", help="Gets the details of a run. Includes admin details if admin.")
...
def list_(offset, problem_alias, rowcount, username, language, status, verdict):
    ctx = get_client()
    api_dict = ctx.run.list(offset=offset, problem_alias=problem_alias, rowcount=rowcount, username=username, language=language, status=status, verdict=verdict)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@school.command(name = "selectSchoolOfTheMonth", help="Selects a certain school as school of the month")
@click.argument("school_id")
...
def refresh(alias, course_alias, token):
    ctx = get_client()
    api_dict = ctx.scoreboard.refresh(alias=alias, course_alias=course_alias, token=token)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@session.command(name = "currentSession", help="Returns information about current session. In order to avoid one full server roundtrip (about ~100msec on each pageload), it also returns the current time to be able to calculate the time delta between the contestant's machine and the server.")
@click.option("--auth-token", default=None, type=str)
...
@submission.command(name = "setFeedback", help="Updates the admin feedback for a submission")
@click.argument("assignment_alias")
@click.argument("course_alias")
...
def set_feedback(assignment_alias, course_alias, feedback, guid):
    ctx = get_client()
    api_dict = ctx.submission.setFeedback(assignment_alias=assignment_alias, course_alias=course_alias, feedback=feedback, guid=guid)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@tag.command(name = "frequentTags", help="Return most frequent public tags of a certain level")
...
def add_members(team_group_alias, usernames):
    ctx = get_client()
    api_dict = ctx.teamsGroup.addMembers(team_group_alias=team_group_alias, usernames=usernames)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def create(alias, description, name, number_of_contestants):
    ctx = get_client()
    api_dict = ctx.teamsGroup.create(alias=alias, description=description, name=name, numberOfContestants=number_of_contestants)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@teams_group.command(name = "list", help="Gets a list of teams groups. This returns an array instead of an object since it is used by typeahead.")
@click.option("--query", default=None, type=str)
def list_(query):
    ctx = get_client()
...
@teams_group.command(name = "removeMember", help="Remove an existing team member of a teams group")
@click.argument("team_group_alias")
@click.argument("username")
def remove_member(team_group_alias, username):
    ctx = get_client()
    api_dict = ctx.teamsGroup.removeMember(team_group_alias=team_group_alias, username=username)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def remove_team(team_group_alias, username_or_email):
    ctx = get_client()
    api_dict = ctx.teamsGroup.removeTeam(team_group_alias=team_group_alias, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@teams_group.command(name = "teamsMembers", help="Get a list of team members of a teams group")
@click.argument("page")
@click.argument("page_size")
@click.argument("team_group_alias")
def teams_members(page, page_size, team_group_alias):
    ctx = get_client()
    api_dict = ctx.teamsGroup.teamsMembers(page=page, page_size=page_size, team_group_alias=team_group_alias)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
def update(alias, description, name, number_of_contestants):
    ctx = get_client()
    api_dict = ctx.teamsGroup.update(alias=alias, description=description, name=name, numberOfContestants=number_of_contestants)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@user.command(name = "acceptPrivacyPolicy", help="Keeps a record of a user who accepts the privacy policy")
@click.argument("privacy_git_object_id")
@click.argument("statement_type")
@click.option("--username", default=None, type=str)
def accept_privacy_policy(privacy_git_object_id, statement_type, username):
    ctx = get_client()
    api_dict = ctx.user.acceptPrivacyPolicy(privacy_git_object_id=privacy_git_object_id, statement_type=statement_type, username=username)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@user.command(name = "associateIdentity", help="Associates an identity to the logged user given the username")
@click.argument("password")
@click.argument("username")
...
def change_password(old_password, username, password, permission_key):
    ctx = get_client()
    api_dict = ctx.user.changePassword(old_password=old_password, username=username, password=password, permission_key=permission_key)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@user.command(name = "coderOfTheMonth", help="Get coder of the month by trying to find it in the table using the first day of the current month. If there's no coder of the month for the given date, calculate it and save it.")
...
@user.command(name = "coderOfTheMonthList", help="Returns the list of coders of the month")
@click.option("--category", default=None)
@click.option("--date", default=None, type=str)
...
@user.command(name = "contestStats", help="Get Contests which a certain user has participated in")
@click.option("--username", default=None, type=str)
...
@user.command(name = "extraInformation", help="Gets extra information of the identity: - last password change request - verify status - birth date to verify the user identity")
@click.argument("email")
...
@user.command(name = "generateGitToken", help="Generate a new gitserver token. This token can be used to authenticate against the gitserver.")
...
def generate_omi_users(auth_token, contest_alias, contest_type, id_, old_password, permission_key, username, change_password, password, username_or_email):
    ctx = get_client()
    api_dict = ctx.user.generateOmiUsers(auth_token=auth_token, contest_alias=contest_alias, contest_type=contest_type, id=id_, old_password=old_password, permission_key=permission_key, username=username, change_password=change_password, password=password, usernameOrEmail=username_or_email)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@user.command(name = "lastPrivacyPolicyAccepted", help="Gets the last privacy policy accepted by user")
@click.option("--username", default=None, type=str)
...
@user.command(name = "listAPITokens", help="Returns a list of all the API tokens associated with the user.")
def list_a_p_i_tokens():
    ctx = get_client()
    api_dict = ctx.user.listAPITokens()
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@user.command(name = "listAssociatedIdentities", help="Get the identities that have been associated to the logged user")
...
@user.command(name = "login", help="Exposes API /user/login Expects in request: user password")
@click.argument("password")
@click.argument("username_or_email")
...
@user.command(name = "mailingListBackfill", help="Registers to the mailing list all users that have not been added before. Admin only")
...
def profile(category, omit_rank, username):
    ctx = get_client()
    api_dict = ctx.user.profile(category=category, omit_rank=omit_rank, username=username)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
...
@user.command(name = "revokeAPIToken", help="Revokes an API token associated with the user.")
@click.argument("name")
...
@user.command(name = "selectCoderOfTheMonth", help="Selects coder of the month for next month.")
@click.argument("username")
@click.option("--category", default=None)
...
@click.option("--username", default=None)
def update(birth_date, country_id, graduation_date, locale, state_id, auth_token, gender, has_competitive_objective, has_learning_objective, has_scholar_objective, has_teaching_objective, hide_problem_tags, is_private, name, scholar_degree, school_id, school_name, username):
    ctx = get_client()
    api_dict = ctx.user.update(birth_date=birth_date, country_id=country_id, graduation_date=graduation_date, locale=locale, state_id=state_id, auth_token=auth_token, gender=gender, has_competitive_objective=has_competitive_objective, has_learning_objective=has_learning_objective, has_scholar_objective=has_scholar_objective, has_teaching_objective=has_teaching_objective, hide_problem_tags=hide_problem_tags, is_private=is_private, name=name, scholar_degree=scholar_degree, school_id=school_id, school_name=school_name, username=username)
    print(json.dumps(api_dict, indent=4, sort_keys=True))

@user.command(name = "updateBasicInfo", help="Update basic user profile info when logged with fb/gool")
@click.argument("password")
@click.argument("username")
...
@click.option("--token", default=None, type=str)
@click.option("--tokens", default=None)
def validate_filter(filter_, problemset_id, auth_token, contest_admin, contest_alias, token, tokens):
    ctx = get_client()
    api_dict = ctx.user.validateFilter(filter=filter_, problemset_id=problemset_id, auth_token=auth_token, contest_admin=contest_admin, contest_alias=contest_alias, token=token, tokens=tokens)
    print(json.dumps(api_dict, indent=4, sort_keys=True))
